...
export class HierarchicalMemory {
  ...
  _score(query, chunk, level = "long") {
    ...
    const chunkKeywords = this._keywords(chunk);
  ...
  retrieveDetailed(query, topK = 3, level = "long") {
    ...
    return pool
      .map((chunk) => ({
        chunk,
        score: this._score(query, chunk, level),
      }))
